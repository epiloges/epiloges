...
/**
 * Detected from the connection string's own host, not from which env var carried it.
 * `POSTGRES_PRISMA_URL` is what Vercel's native Supabase integration names the pooled
 * connection string when a Supabase project is connected via the dashboard — but some
 * setups (manually pasting the pooler URL, or older integration versions) put that same
 * value straight into `DATABASE_URL`. Keying off the env var name alone missed that case
 * entirely: `DATABASE_URL` would be set, the relaxation would silently skip, and every
 * query would fail its TLS handshake in production. See the matching fallback in
 * prisma.config.ts for the direct-connection half of this.
 */
