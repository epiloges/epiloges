 ...
 * The last circle is deliberately clipped at the container edge (no `snap-mandatory`
 * scroll-padding covering it) — a partial circle is the visual cue that there's more to
 * swipe to, the same trick Instagram's own story tray uses.
 */
export function CategoryStories({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <div className="relative border-b border-border">
      <div
        className="flex shrink-0 snap-x snap-mandatory gap-3 overflow-x-auto px-4 py-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
          ...
          >
            <span className="rounded-full bg-gradient-to-tr from-[#f9a13f] via-[#e0356b] to-luxe-purple p-[2.5px] transition-transform duration-200 group-hover:scale-105">
              <span className="block rounded-full bg-luxe-white p-[2px]">
                <span className="relative block size-17 overflow-hidden rounded-full bg-luxe-gray-light">
                  <Image
            ...
            {/* w-[72px], a touch wider than the circle: without it a long label (e.g.
                "Denim Jackets") widens the whole flex item past its circle, throwing off
                the otherwise-even rhythm of the row. Truncates instead, same as Instagram
                does with long usernames. */}
            <span className="w-[72px] truncate text-center text-xs font-medium text-luxe-black">
              {category.label}
            </span>
          </Link>
        ))}
      </div>
      {/* Fades the trailing edge of whichever circle sits at the boundary, reinforcing the
          clipped-circle hint that the row keeps going. Pointer-events-none so it never
          blocks the swipe/scroll underneath it. */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-luxe-white to-transparent"
      />
    </div>
  );
}
