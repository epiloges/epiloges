...
/**
 * The store's own short article codes ("7251", "TR-1") are what customers quote on the
 * phone. Codes generated by an import ("WC-BAG-VALENTINO-BAG-VBS3KK48R") are not — they
 * are internal, and printing them under the title looks like a data leak.
 */
function isCustomerFacingSku(sku: string): boolean {
  return !sku.startsWith("WC-") && sku.length <= 12;
}
...
export function PurchasePanel({ product, categoryName, rates }: PurchasePanelProps) {
  ...
  // One size (a bag, a belt) is not a choice — pre-select it so the button is live at once.
  const [selectedSize, setSelectedSize] = useState<string | null>(() =>
    product.sizes.length === 1 && isSizePurchasable(product, product.sizes[0].name) ? product.sizes[0].name : null
  );
  ...
  return (
    <div className="flex flex-col">
      ...
      {product.sku && isCustomerFacingSku(product.sku) ? (
